...
<?php elseif (in_array($step,[2,3],true)): $checks=$step===2?$phpChecks:$permissionChecks; ?><h1><?= $escape($t($step===2?'requirements':'permissions')) ?></h1><div class="checks"><?php foreach($checks as $label=>$passed): ?><div class="check"><span><?= $escape($label) ?></span><strong class="<?= $passed?'ok':'bad' ?>"><?= $escape($t($passed?'passed':'failed')) ?></strong></div><?php endforeach; ?></div><form method="post"><input type="hidden" name="_csrf" value="<?= $escape($_SESSION['installer_csrf']) ?>"><div class="actions"><button class="button" <?= in_array(false,$checks,true)?'disabled':'' ?>><?= $escape($t('next')) ?></button></div></form>
<?php elseif ($step === 4): ?><h1><?= $escape($t('database')) ?></h1><form method="post"><input type="hidden" name="_csrf" value="<?= $escape($_SESSION['installer_csrf']) ?>"><div class="grid"><label>Host<input name="host" value="127.0.0.1" required></label><label>Port<input name="port" type="number" value="3306" required></label><label>Database<input name="name" value="yii2_website" pattern="[A-Za-z0-9_]+" required></label><label>User<input name="user" value="root" required></label></div><label>Password<input name="password" type="password" autocomplete="new-password"></label><div class="actions"><button class="button"><?= $escape($t('next')) ?></button></div></form>
<?php elseif ($step === 5): ?><h1><?= $escape($t('connection')) ?></h1><p class="muted">Database: <?= $escape($_SESSION['installer_database']['name'] ?? '') ?></p><form method="post"><input type="hidden" name="_csrf" value="<?= $escape($_SESSION['installer_csrf']) ?>"><div class="actions"><button class="button"><?= $escape($t('test')) ?></button></div></form>
<?php elseif ($step === 6): ?><h1><?= $escape($t('migrations')) ?></h1><p class="muted">Yii migrations create all tables, indexes and RBAC data. You can safely retry this step after an error.</p><form method="post"><input type="hidden" name="_csrf" value="<?= $escape($_SESSION['installer_csrf']) ?>"><div class="actions"><button class="button"><?= $escape($t('run')) ?></button></div></form>
<?php elseif ($step === 7): ?><h1><?= $escape($t('admin')) ?></h1><form method="post"><input type="hidden" name="_csrf" value="<?= $escape($_SESSION['installer_csrf']) ?>"><label>Username<input name="username" value="admin" minlength="3" required></label><label>Email<input name="email" type="email" required></label><label>Password<input name="password" type="password" minlength="12" autocomplete="new-password" required><small class="muted">12+ characters with uppercase, lowercase, number and symbol</small></label><div class="actions"><button class="button"><?= $escape($t('next')) ?></button></div></form>
<?php elseif ($step === 8): ?><h1><?= $escape($t('site')) ?></h1><form method="post"><input type="hidden" name="_csrf" value="<?= $escape($_SESSION['installer_csrf']) ?>"><label>Site name<input name="name" required></label><label>Site URL<input name="url" type="url" value="<?= $escape((!empty($_SERVER['HTTPS'])?'https':'http').'://'.($_SERVER['HTTP_HOST']??'127.0.0.1:8080')) ?>" required></label><label>Address<input name="address"></label><div class="grid"><label>Site language<select name="language"><option value="fa">فارسی</option><option value="en">English</option></select></label><label>Admin language<select name="admin_language"><option value="fa">فارسی</option><option value="en">English</option></select></label></div><div class="actions"><button class="button"><?= $escape($t('save')) ?></button></div></form>

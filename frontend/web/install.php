...
require $root . '/common/installer/InstallerWorkflow.php';
...
$isHttps = InstallerWorkflow::isHttps($_SERVER['HTTPS'] ?? null);
...
$wizardActions = static function (string $primary, bool $disabled = false) use ($t, $escape): string {
    $confirmation = $escape(json_encode($t('restart_confirm'), JSON_UNESCAPED_UNICODE | JSON_HEX_APOS | JSON_HEX_QUOT));
    return '<div class="actions">'
        . '<input type="hidden" name="restart_confirmed" value="0">'
        . '<button class="button button-secondary button-restart" type="submit" name="action" value="restart" formnovalidate '
        . 'onclick="if(!window.confirm(' . $confirmation . '))return false;this.form.elements.restart_confirmed.value=\'1\';">' . $escape($t('restart')) . '</button>'
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (!in_array($action, ['next', 'back', 'restart'], true)) {
            throw new RuntimeException('Invalid installer action.');
        }
        if ($action === 'restart') {
            if (!InstallerWorkflow::restartConfirmed($_POST['restart_confirmed'] ?? null)) {
                throw new RuntimeException($t('restart_required'));
            }
            $_SESSION = [];
            ...
            $step = InstallerWorkflow::goBack($step, $_SESSION);
        } elseif ($step === 1) {
            session_regenerate_id(true);
            $language = in_array($_POST['language'] ?? '', ['fa', 'en'], true) ? $_POST['language'] : 'fa';
            $_SESSION['installer_language'] = $language;
            $step = 2;
        } elseif ($step === 2 && InstallerWorkflow::checksPassed($phpChecks)) {
            $step = 3;
        } elseif ($step === 3 && InstallerWorkflow::checksPassed($permissionChecks)) {
            $step = 4;
        } elseif ($step === 4) {
            ...
            $database = [
                'host' => trim((string) ($_POST['host'] ?? '')), 'port' => (int) ($_POST['port'] ?? 0),
                'name' => trim((string) ($_POST['name'] ?? '')), 'user' => trim((string) ($_POST['user'] ?? '')),
                'password' => $password,
            ];
            $installer->validateDatabase($database);
            ...
            $_SESSION['installer_admin_draft'] = ['username' => $admin['username'], 'email' => $admin['email']];
            ...
            $_SESSION['installer_site_draft'] = $site;
            ...
            $installer->lock($site);
            $_SESSION = ['installer_step' => InstallerWorkflow::FINISHED_STEP, 'installer_language' => $language];
            $step = InstallerWorkflow::FINISHED_STEP;
        }
...
if ($step >= 7 && isset($_SESSION['installer_database'])) {
    try {
        if (!$installer->databaseSchemaExists($_SESSION['installer_database'])) {
            $step = 6;
            unset($_SESSION['installer_admin']);
            $_SESSION['installer_step'] = $step;
        }
    } catch (Throwable $exception) {
        // The migration step will show the actionable connection/schema error.
    }
}
...
<body><main class="shell"><div class="brand"><span class="mark">B</span><?= $escape($t('title')) ?></div><section class="card"><div class="steps"><?php for ($i=1;$i<=InstallerWorkflow::FINISHED_STEP;$i++): ?><i class="<?= $i <= $step ? 'done' : '' ?>"></i><?php endfor; ?></div>
...
<?php elseif ($step === 7): $adminDraft=$_SESSION['installer_admin_draft']??[]; ?><h1><?= $escape($t('admin')) ?></h1><form method="post"><input type="hidden" name="_csrf" value="<?= $escape($_SESSION['installer_csrf']) ?>"><label>Username<input name="username" value="<?= $escape($adminDraft['username']??'admin') ?>" minlength="3" required></label><label>Email<input name="email" type="email" value="<?= $escape($adminDraft['email']??'') ?>" required></label><label>Password<input name="password" type="password" minlength="12" autocomplete="new-password" required><small class="muted">12+ characters with uppercase, lowercase, number and symbol</small></label><?= $wizardActions($t('next')) ?></form>
<?php elseif ($step === 8): $siteDraft=$_SESSION['installer_site_draft']??[]; $defaultUrl=($isHttps?'https':'http').'://'.($_SERVER['HTTP_HOST']??'127.0.0.1:8080'); ?><h1><?= $escape($t('site')) ?></h1><form method="post"><input type="hidden" name="_csrf" value="<?= $escape($_SESSION['installer_csrf']) ?>"><label>Site name<input name="name" value="<?= $escape($siteDraft['name']??'') ?>" required></label><label>Site URL<input name="url" type="url" value="<?= $escape($siteDraft['url']??$defaultUrl) ?>" required></label><label>Address<input name="address" value="<?= $escape($siteDraft['address']??'') ?>"></label><div class="grid"><label>Site language<select name="language"><option value="fa" <?= ($siteDraft['language']??'fa')==='fa'?'selected':'' ?>>فارسی</option><option value="en" <?= ($siteDraft['language']??'fa')==='en'?'selected':'' ?>>English</option></select></label><label>Admin language<select name="admin_language"><option value="fa" <?= ($siteDraft['admin_language']??'fa')==='fa'?'selected':'' ?>>فارسی</option><option value="en" <?= ($siteDraft['admin_language']??'fa')==='en'?'selected':'' ?>>English</option></select></label></div><?= $wizardActions($t('save')) ?></form>
